...
* @subpackage    Backend
*/
class conditionsext extends Survey_Common_Action
{
     ...
     * @return void
     */
    public function index($subaction, $iSurveyID = null, $gid = null, $strg_id = null)
    {
        ...
        $aData['strategybar']['closebutton']['url'] = 'admin/strategies/sa/view/surveyid/'.$iSurveyID.'/strg_id/'.$strg_id; // Close button
        $aData['strategybar']['buttons']['conditions'] = true;
        
        switch ($subaction) {
            case 'editconditionsform':
                $aData['strategybar']['buttons']['condition']['edit'] = true;
                break;
            
            case 'conditions':
                $aData['strategybar']['buttons']['condition']['conditions'] = true;
                break;
            
            case 'copyconditionsform':
                $aData['strategybar']['buttons']['condition']['copyconditionsform'] = true;
                break;
    ...
    if (!isset($strg_id)) { $strg_id = returnGlobal('strg_id'); }

    $language = Survey::model()->findByPk($iSurveyID)->language;
    $this->language = $language;
    $questionRowsTemp = $this->getQuestionRows();
    $questionlistTemp = $this->getQuestionListExt($questionRowsTemp);
    $qidTemp=end($questionlistTemp);
    if (!isset($qid)) { $qid = $qidTemp; }
    ...
    if (!isset($strg_id) || !$strg_id) {
        Yii::app()->setFlashMessage(gT('You have not selected a strategy'), 'error');
    ...
    $extraGetParams = "";
    if (isset($strg_id) && isset($gid)) {
        $extraGetParams = "/gid/{$gid}/strg_id/{$strg_id}";
    }
    ...
    list($strategytitle, $sCurrentFullStrategyText) = $this->getStrategyTitleAndText($strg_id);
    ...
    $questionlist = $this->getQuestionListExt($questionRows);
    $postquestionlist = $this->getPostQuestionListExt($questionRows);
...
//Build the array used for the questionNav and copyTo select boxes
    ...
    $args['sCurrentFullQuestionText'] = $sCurrentFullStrategyText;
    $args['questiontitle'] = $strategytitle;
    $args['gid'] = $gid;
    ...
    $oStrategy = Strategy::model()->find('strg_id=:strg_id', array(':strg_id'=>$strg_id));
    $aData['oStrategy'] = $oStrategy;
    ...
    $scenarios = $this->getAllScenarios($strg_id);
    ...
    '/admin/conditionsext/sa/quickAddCondition',
    array(
    ...
    $subaction == 'copyconditionsform' || $subaction == 'copyconditions' || $subaction == 'conditions'
        ...
        $aData['conditionsoutput_action_error'] = $conditionsoutput_action_error;
        $aData['javascriptpre'] = $javascriptpre;
        $aData['sCurrentQuestionText'] = $strategytitle.': '.viewHelper::flatEllipsizeText($sCurrentFullStrategyText, true, '120');
        ...
        if ($scenariocount > 0) {
            ...
            foreach ($scenarios as $scenarionr) {
                ...
                '/admin/conditionsext/includes/conditions_scenario',
                $aData,
                true
                );
                */
                ...
                $conditionscount = StrgCondition::model()->getConditionCount($strg_id, $this->language, $scenarionr);
                $conditions = StrgCondition::model()->getConditions($strg_id, $this->language, $scenarionr);
                $conditionscounttoken = StrgCondition::model()->getConditionCountToken($strg_id, $scenarionr);
                $resulttoken = StrgCondition::model()->getConditionsToken($strg_id, $scenarionr);
                ...
                if ($conditionscount > 0) {
                    ...
                    foreach ($aConditionsMerged as $rows) {
                        ...
                        //$aViewUrls['output'] .= CHtml::form(array("/admin/conditionsext/sa/index/subaction/{$subaction}/surveyid/{$iSurveyID}/gid/{$gid}/qid/{$qid}/"), 'post', array('id'=>"conditionaction{$rows['cid']}",'name'=>"conditionaction{$rows['cid']}"));
...
// show single condition action buttons in edit mode
                        ...
                        $data['editButtons'] = $this->getController()->renderPartial('/admin/conditionsext/includes/conditions_edit', $aData, true);
                    ...
                    '/admin/conditionsext/includes/condition',
                    $data,
                    true
                    );
...
// no condition ==> disable delete all conditions button, and display a simple comment
        // no_conditions
        $aViewUrls['output'] = $this->getController()->renderPartial('/admin/conditionsext/no_condition', $aData, true);
    }
...
|| $subaction == "copyconditions") {
    $aViewUrls['output'] .= $this->getCopyForm($strg_id, $gid, $conditionsList, $pquestions);
}
...
$this->_renderWrappedTemplate('conditionsext', $aViewUrls, $aData);
...
protected function resetSurveyLogic($iSurveyID)
{
    if (empty(Yii::app()->request->getPost('ok'))) {
        ...
        $content = $this->getController()->renderPartial('/admin/conditionsext/deleteAllConditions', $data, true);
        ...
        StrgCondition::model()->deleteRecords("qid in (select qid from {{questions}} where sid={$iSurveyID})");
...
protected function insertCondition(array $args)
{
    ...
    if ($p_canswers) {
        ...
        foreach ($p_canswers as $ca) {
            ...
            $result = StrgCondition::model()->findAllByAttributes($condition_data);
            
            $count_caseinsensitivedupes = count($result);
            
            if ($count_caseinsensitivedupes == 0) {
                $results[] = StrgCondition::model()->insertRecords($condition_data); ;
            }
        }
        ...
        if ($posted_condition_value !=='') {
            ...
            $result = StrgCondition::model()->insertRecords($condition_data);
        }
        if($result) {
            Yii::app()->setFlashMessage(gT('Condition added.'), 'success');
        } else {
    ...
    LimeExpressionManager::UpgradeConditionsextToRelevance(null, $strg_id);
    
    $this->redirectToConditionStart($strg_id, $gid);
}
...
protected function updateCondition(array $args)
{
    ...
    if ($p_canswers) {
        foreach ($p_canswers as $ca) {
            ...
            $results[] = StrgCondition::model()->insertRecords($updated_data, true, array('cid'=>$p_cid));
        }
        ...
        if ($posted_condition_value !== '') {
            ...
            $result = StrgCondition::model()->insertRecords($updated_data, true, array('cid'=>$p_cid));
        }
        if ($result) {
            Yii::app()->setFlashMessage(gT('Condition updated.'), 'success');
        } else {
...
protected function renumberScenarios(array $args)
{
    ...
    $query = "SELECT DISTINCT scenario FROM {{strg_conditions}} WHERE strg_id=:strg_id ORDER BY scenario";
    $result = Yii::app()->db->createCommand($query)->bindParam(":strg_id", $strg_id, PDO::PARAM_INT)->query() or safeDie("Couldn't select scenario<br />$query<br />");
    $newindex = 1;
    
    foreach ($result->readAll() as $srow) {
        StrgCondition::model()->insertRecords(array('scenario'=>$newindex), true, array('strg_id'=>$strg_id, 'scenario'=>$srow['scenario']));
        $newindex++;
    }
    LimeExpressionManager::UpgradeConditionsextToRelevance(null, $strg_id);
...
protected function applySubaction($p_subaction, array $args)
{
    ...
    switch ($p_subaction) {
        ...
        case "delete":
            LimeExpressionManager::RevertUpgradeConditionsextToRelevance(null, $strg_id); // in case deleted the last condition
            StrgCondition::model()->deleteRecords(array('cid'=>$p_cid));
            LimeExpressionManager::UpgradeConditionsextToRelevance(null, $strg_id);
            $this->redirectToConditionStart($strg_id, $gid);
            break;
        ...
        case "deletescenario":
            LimeExpressionManager::RevertUpgradeConditionsextToRelevance(null, $strg_id); // in case deleted the last condition
            StrgCondition::model()->deleteRecords(array('strg_id'=>$strg_id, 'scenario'=>$p_scenario));
            LimeExpressionManager::UpgradeConditionsextToRelevance(null, $strg_id);
            $this->redirectToConditionStart($strg_id, $gid);
            break;
        ...
        case "updatescenario":
            ...
            StrgCondition::model()->insertRecords(array('scenario'=>$p_newscenarionum), true, array(
        ...
        case "deleteallconditions":
            LimeExpressionManager::RevertUpgradeConditionsextToRelevance(null, $strg_id); // in case deleted the last condition
            StrgCondition::model()->deleteRecords(array('strg_id'=>$strg_id));
...
protected function getStrategyTitleAndText($strg_id)
{
    $strategy = Strategy::model()->with('groups')->findByAttributes(array(
    'strg_id' => $strg_id,
    'language' => $this->language
    ));
    return array($strategy['strg_name'], $strategy['strg_name']);
}
...
protected function getQuestionListExt(array $qrows)
{
    $questionlist = array();
    // Go through each question until we reach the current one
        foreach ($qrows as $qrow) {
...
* @return array
*/
protected function getPostQuestionListExt(array $qrows)
{
    $position = "before";
...
protected function getCopyForm($qid, $gid, array $conditionsList, array $pquestions)
{
    ...
    '/admin/conditionsext/sa/index/subaction/copyconditions/',
    array(
    'surveyid' => $this->iSurveyID,
    'gid' => $gid,
    'qid' => $qid
    ...
    '/admin/conditionsext/includes/copyform',
    $data,
    true
    );
}
...
protected function getEditConditionForm(array $args)
{
    ...
    $result .= $this->getController()->renderPartial('/admin/conditionsext/includes/form_editconditions_header', $data, true);
...
protected function getQuickAddConditionForm(array $args)
{
    ...
    $html = $this->getController()->renderPartial('/admin/conditionsext/includes/quickAddConditionForm', $data, true);
    return $html;
}
...
protected function getQuestionNavOptions($theserows, $postrows, $args)
{
    ...
    return $this->getController()->renderPartial('/admin/conditionsext/includes/navigator', $data, true);
}
...
protected function createNavigatorUrl($gid, $qid)
{
    ...
    '/admin/conditionsext/sa/index/subaction/editconditionsform/',
    array(
    'surveyid' => $this->iSurveyID,
    'gid' => $gid,
    'qid' => $qid
...
*/
protected function getAllScenarios($strg_id)
{
    $criteria = new CDbCriteria;
    $criteria->select = 'scenario'; // only select the 'scenario' column
    $criteria->condition = 'strg_id=:strg_id';
    $criteria->params = array(':strg_id'=>$strg_id);
    $criteria->order = 'scenario';
    $criteria->group = 'scenario';
    
    return StrgCondition::model()->findAll($criteria);
}
...
* @return void
*/
protected function redirectToConditionStart($strg_id, $gid)
{
    $url = $this->getcontroller()->createUrl(
    '/admin/conditionsext/sa/index/subaction/editconditionsform/',
    array(
