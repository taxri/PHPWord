...
class SurveyAdmin extends Survey_Common_Action
{
    ...
    * @return mixed                 null or array
    ...
    /**
     * Load list strategies view for a specified by $iSurveyID
     *
     * @access public
     * @param mixed $surveyid The survey ID
     * @return void
     */
    public function liststrategies($surveyid)
    {
        $iSurveyID = sanitize_int($surveyid);
        $survey = Survey::model()->findByPk($iSurveyID);
        ...
        $aData['surveyid']                                   = $iSurveyID;
        $aData['display']['menu_bars']['liststrategies'] = true;
        $aData['sidemenu']['questiongroups']                 = true;
        $aData['sidemenu']['listquestiongroups']             = true;
        $aData['surveybar']['buttons']['newstrategy']           = true;

        //title = $aData['title_bar']['title'] + $aData['subaction']
        $aData['title_bar']['title']                         = $survey->currentLanguageSettings->surveyls_title." (".gT("ID").":".$iSurveyID.")";
        $aData['subaction']                                  = gT("Strategies in this survey");

        //$aData['questiongroupbar']                 = true;
    ...
    private function _getTextEditData($survey)
    {
        ...
        if ($survey->sid) {
            ...
            $aLanguageData['aSurveyLanguageSettings'] = [
                ...
                'surveyls_template_docx' => '',
                'surveyls_template_docx_suffix' => '',
    ...
    public function insert($iSurveyID = null)
    {
        if (Permission::model()->hasGlobalPermission('surveys', 'create')) {
            ...
            $aInsertData = array(
                ...
                'surveyls_template_docx' => Yii::app()->request->getPost('tempdocx', ''),
                'surveyls_template_docx_suffix' => Yii::app()->request->getPost('tempdocxsuf', ''),
