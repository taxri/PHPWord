...
class DocWriter extends Writer
{
    ...
    protected function outputRecord($headers, $values, FormattingOptions $oOptions, SurveyObj $oSurvey, $mask)
    {
        ...
        if (strstr($oSurvey->languageSettings['surveyls_template_docx'],'docx')){
            ...
            $sSuffix = "out";
            $sDocxNameTmp = str_replace(".docx", "", $sDocxName);
            $sOutDocxName = "{$sDocxNameTmp}_out.docx";


            $counter = 0;
            foreach ($headers as $header) {
                if (strstr($header, $oSurvey->languageSettings['surveyls_template_docx_suffix'])){
                    $sSuffix = $values[$counter];
                    break;
                }
                $counter++;
            }
            if (strlen($sSuffix)>0)
            {
                $sOutDocxName =  "{$sDocxNameTmp}_{$sSuffix}.docx";
            }
